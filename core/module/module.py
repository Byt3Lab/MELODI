...
class Module:
    ...
    async def notify_event(self, event_name: str, data: Any = None):
        """Déclenche un événement propre à ce module."""
        await self.app.event_listener.notify_event(self.dirname, event_name, data)
    ...
    def add_hook(self, target_module: str, hook_name: str, hook: Callable):
        ...
        self.app.hook_manager.register_hook(target_module, hook_name, hook)

    async def execute_hook(self, hook_name: str, payload=None):
        """Déclenche un hook pour informer le reste de l'application."""
        # Un module déclenche toujours ses propres hooks
        return await self.app.hook_manager.execute_hook(self.dirname, hook_name, payload)
