...
- **Router**: Handles web and API routing (`core/router/`). It wraps Quart's routing system (Asynchronous).
...
1.  Request hits the Quart server (ASGI).
2.  `QuartAdapter` routes the request to the appropriate `WebRouter` or `APIRouter`.
3.  The router executes the **async** view function defined in the module.
4.  The view function may **await** services or the database operations.
